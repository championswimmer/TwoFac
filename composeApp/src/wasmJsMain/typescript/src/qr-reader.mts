...
const decodeBlobWithJsQr = async (blob: Blob): Promise<ClipboardReadResult> => {
  ...
  try {
    ...
    const name = (error as { name?: string } | null)?.name ?? "DecodeError";
...
const decodeFirstImageBlob = async (blobPromises: Promise<Blob | null>[]): Promise<ClipboardReadResult> => {
...
const extractImageBlobPromisesFromClipboardItems = (clipboardItems: ClipboardItems | undefined): Promise<Blob | null>[] => {
  const blobPromises: Promise<Blob | null>[] = [];
  if (!clipboardItems) return blobPromises;
...
const extractImageBlobPromisesFromPasteData = (clipboardData: DataTransfer | null): Promise<Blob | null>[] => {
  const blobPromises: Promise<Blob | null>[] = [];
...
const readUsingAsyncClipboardApi = async (): Promise<ClipboardReadResult> => {
  ...
  try {
    ...
    const name = (error as { name?: string } | null)?.name ?? "ClipboardReadError";
...
const readUsingPasteEvent = (
  ...
  new Promise((resolve) => {
    ...
    const finalize = (result: ClipboardReadResult) => {
      if (completed) return;
      completed = true;
    ...
    const onPaste = (event: ClipboardEvent) => {
      ...
      decodeFirstImageBlob(extractImageBlobPromisesFromPasteData(clipboardData))
        .then(finalize)
        .catch((error) => {
          const name = (error as { name?: string } | null)?.name ?? "PasteDecodeError";
    ...
    timeoutHandle = setTimeout(
      () => finalize({ status: timeoutStatus, decodedPayload: null, message: timeoutMessage }),
      timeoutMs,
    );
  });

export const readQRCodeFromClipboard = async (pasteTimeoutMs: number): Promise<ClipboardReadResult> => {
